...
/* This script has Google no captcha recaptcha. In order to configure it go down to line 981 and replace all the text that reads "YOUR KEY GOES HERE".
...
Step 1: Put the form on your webpage (if not using an existing form)
...
Put the form on your webpage (if not using an existing form).
...
$my_email = "crystalignchiro@gmail.com";
...
$subject = "Comments from crystalignchiropracric.com Contact Form";
...
<?php if(isset($form_field_name)){print $form_field_name;}else{print "";} ?>
...
<input type="text" name="Street_Address" value="<?php if(isset($Street_Address)){print $Street_Address;}else{print "";} ?>">
...
foreach($_SESSION['formtoemail_form_errors'] as $form_error_value){print "<b>" . $form_error_value . "</b><br>";}
...
$fields_to_ignore = array('Submit','submit','recaptcha_challenge_field','recaptcha_response_field','captcha_code','g-recaptcha-response');
...
if(!$captcha){
        ...
        $response=file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=6LfOwO0UAAAAAAbfFquosUNZhbWcqkAMtBmciwDu&response=".$captcha."&remoteip=".$_SERVER['REMOTE_ADDR']);
...
if(count($errors) && $show_errors_on_form_page == 0){foreach($errors as $value){print "$value<br>";} exit;}
...
<br>Your form submission has been received.
